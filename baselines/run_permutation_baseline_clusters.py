"""
Run do_permutation_baseline_clusters (Spearman) over all model × dataset combos.

Mirrors run_permutation_baseline.py but operates on cross-layer neuron clusters
rather than whole layers.  If cross_layer_clustering_results_all_neurons.json is
missing for a combo, clustering is run first.
...
set UPDATE_FEATURES to a comma-separated list:
    ...
    UPDATE_FEATURES=spectral_centroid,spectral_bandwidth python3 run_permutation_baseline_clusters.py
...
Results saved to:
  results_500/{n_clusters}_cluster/{model}/{dataset}/permutation_baseline_clusters.json
"""
import os
import sys
import json
...
pca_components = 2
device      = "cuda"
N_PERMS     = 500     # only used when use_global_null=False
THRESHOLD   = 0.15
...
for model_name in models:
    ...
    for dataset_name, dataset in datasets_dict.items():
        ...
        out_file = output_dir / "permutation_baseline_clusters.json"
        if out_file.exists() and not FORCE:
            with open(out_file) as _f:
                _existing = json.load(_f)
            # existing is {section: {cluster: {prop: stats}}}; check if all props
            # are present across every section and cluster
            all_done = all(
                prop_key in cluster_data
            ...
            if all_done:
                ...
                continue
            print(f"  [{dataset_name}] incomplete, continuing...")
        ...
        analyser.do_permutation_baseline_clusters(
            ...
            update=True,
            force=FORCE,
            use_global_null=USE_GLOBAL_NULL,
        )

print("\n✓ All Spearman cluster permutation baselines complete.")
