"""
Run do_permutation_baseline_nonlinear (MLP probe) over all model × dataset combos.

Mirrors run_permutation_baseline.py but uses a small MLP to test nonlinear
decodability of each feature from each layer's activations, with a shuffled-label
null distribution for significance.  Optionally also fits a linear probe for
the three-way comparison (Spearman | linear probe R² | nonlinear probe R²).
...
set UPDATE_FEATURES to a comma-separated list:
    ...
    UPDATE_FEATURES=spectral_centroid,spectral_bandwidth python3 run_permutation_baseline_nonlinear.py
...
Results saved to:
  results/{n_clusters}_cluster/{model}/{dataset}/permutation_baseline_nonlinear.json
"""
import os
import sys
import json
...
for model_name in models:
    ...
    for dataset_name, dataset in datasets_dict.items():
        ...
        perm_file = output_dir / "permutation_baseline_nonlinear.json"
        if perm_file.exists() and not FORCE:
            with open(perm_file) as _f:
                _existing = json.load(_f)
            missing = [p for p in prop if p not in _existing]
            if not missing:
...
print("\n✓ All nonlinear permutation baselines complete.")
