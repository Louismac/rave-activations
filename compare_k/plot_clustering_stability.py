...
def load_data(model, dataset):
    data = {}
    for k in K_RANGE:
        f = base_path / f"{k}_cluster" / model / dataset / 'cross_layer_cluster_correlation_all_neurons.json'
        if f.exists():
            with open(f) as fh:
                data[k] = json.load(fh)
    return data
...
fig.suptitle("Cluster stability across k  (mean ± SEM, 12 model × dataset combos)",
             fontsize=13, y=1.02)
plt.tight_layout()
out = base_path.parent / "compare_k" / "clustering_stability_across_k.png"
