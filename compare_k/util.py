...
def load_data(i):
    ...
    base_path = base_path / "results_500" 
                    ...
                    if section in data:
                        ...
                        for cluster_key, cluster_info in section_data.items():
                            if 'properties' in cluster_info:
                                ...
                                # Extract spectral centroid correlation
                                sc_corr = props.get('spectral_centroid', {}).get('mean_correlation', 0)
                                sc_max = props.get('spectral_centroid', {}).get('max_correlation', 0)
                                sc_responsive = props.get('spectral_centroid', {}).get('n_responsive_neurons', 0)

                                # Extract spectral bandwidth correlation
                                sb_corr = props.get('spectral_bandwidth', {}).get('mean_correlation', 0)
                                sb_max = props.get('spectral_bandwidth', {}).get('max_correlation', 0)
                                sb_responsive = props.get('spectral_bandwidth', {}).get('n_responsive_neurons', 0)
                                ...
                                cluster_correlations.append({
                                    ...
                                    'spectral_centroid_mean_corr': abs(sc_corr),
                                    'spectral_centroid_max_corr': abs(sc_max),
                                    'spectral_centroid_responsive': sc_responsive,
                                    'spectral_bandwidth_mean_corr': abs(sb_corr),
                                    'spectral_bandwidth_max_corr': abs(sb_max),
                                    'spectral_bandwidth_responsive': sb_responsive
                                })
            ...
            if layer_file.exists():
                ...
                for _, row in df.iterrows():
                    ...
                    layer_correlations.append({
                        ...
                        'spectral_centroid_corr': abs(row['SC Corr']),
                        'spectral_bandwidth_corr': abs(row['SB Corr'])
                    })
